...
class Tort3D(PhaseDescriptor3D):
    ...
    def make_singlephase_descriptor(**kwargs) -> callable:
        ...
        def compute_descriptor(microstructure: np.ndarray) -> np.ndarray:
            print(f'{microstructure.shape=}')
            print(type(microstructure))
            print(f'{np.unique(microstructure)=}')
...
def skeletonize3D(_image: np.ndarray, _direction_flow: int, _starting_depth: int =0, 
                  _plot_skel: bool = False, _fname: bool = None, _DIRname: bool = None):
    '''
    ...
    return medial_axis_image, nrows, ncolumns, ndepth, cum_number_medial_each_slide, number_medial_each_slide, skeleton_volume_axis_index
...
# @njit
def find_starting_points(_image: np.ndarray, _junctions_index: np.ndarray, _starting_depth: int,
                         nrows: int, ncolumns: int, _reduce_SP: float = 1.0, _plot_SP: bool = False, 
                         _fname: str = None, _DIRname: str = None):
    '''
    ...
    while len(temp_SP_loc_rad) >= 2:
        ...
        remove_dist=np.sqrt(((current_entry[0]-temp_SP_loc_rad[:,0])**2)+ \
                ((current_entry[1]-temp_SP_loc_rad[:,1])**2)+((current_entry[2]-temp_SP_loc_rad[:,2])**2))- \
...
def running_paths(_image: np.ndarray, _SP_loc_rad_shuffle: np.ndarray, nrows:int,
                  ncolumns: int, ndepth: int, cum_number_medial_each_slide: np.ndarray, 
                  number_medial_each_slide: np.ndarray, skeleton_volume_axis_index:np.ndarray,
                  _connect_search: int =26, _plot: bool =False, _DIRname: str =None, _fname: str=None):
    '''
    ...
    for path in range(len(_SP_loc_rad_shuffle)):
        ...
        next_move_index = next_move_location[0] + nrows*next_move_location[1] + nrows*ncolumns*next_move_location[2]
        ...
        while (next_move_location[2] < (ndepth - 1)) and path_corrector == 1: #len(image[0]) - 1)
            ...
            connect_voxel_index = connect_voxel_coor[:, 0] + nrows*connect_voxel_coor[:, 1] + nrows*ncolumns*connect_voxel_coor[:, 2]
            ...
            next_move_match = np.isin(connect_voxel_index, skeleton_volume_axis_index[x1:x2]) # boolean array with matching voxels
            ...
            if len(connect_voxel_index) > 0:
                ...
                number_medial_each_slide[int(next_move_location[2])+2] = number_medial_each_slide[int(next_move_location[2])+2] -1
        ...
        if len(path_index) > 1 and path_corrector != 0:
            ...
            _image[path_location[:,0], path_location[:,1], path_location[:,2]] = 4 + path # mark path with specific number
            

    all_paths_length = [length for length in all_paths_length if length > 1]    # delete all "paths" that have the length "1"
...
def statistical_tort(_all_paths_length: list, nrows: int, _plot: bool = False, 
                     _SP_loc_rad: np.ndarray =None, _fname: str =None, _DIRname: str =None):
    '''
...
def plot_tort_distribution(SP_loc_rad_shuffle: np.ndarray, tort_values: np.ndarray,
...
def get_neighboring_voxels(nrows,ncolumns,ndepth,coord,connect):
    ...
    def neighbour(coord, nx = 0, ny = 0, nz = 0):
        ...
        if coord.ndim > 1:
            ...
            false_rows_2 = ((neigbour[:,0] >= nrows) | (neigbour[:,1] >= ncolumns) |
                                (neigbour[:,2] >= ndepth)) # are there any values > size?
        else:
            neigbour = np.transpose(np.array([coord[0] + nx, coord[1] + ny, coord[2] + nz]))
            # search for unrealistic results and change row to initial coordinate  
    ...
    n_1  = neighbour(coord, nx = -1, ny = -1, nz = -1) # neighbor of the *previous slide* with on the *top height and left position*
    n_2  = neighbour(coord, nx = -1, ny =  0, nz = -1) # neighbor of the *previous slide* with on the *same height and left position*
    n_3  = neighbour(coord, nx = -1, ny = +1, nz = -1) # neighbor of the *previous slide* with on the *bottom height and left position*
    n_4  = neighbour(coord, nx =  0, ny = -1, nz = -1) # neighbor of the *previous slide* with on the *top height and mddle position*
    n_5  = neighbour(coord, nx =  0, ny =  0, nz = -1) # neighbor of the *previous slide* with on the *same height and mddle position*
    n_6  = neighbour(coord, nx =  0, ny = +1, nz = -1) # neighbor of the *previous slide* with on the *bottom height and mddle position*
    n_7  = neighbour(coord, nx = +1, ny = -1, nz = -1) # neighbor of the *previous slide* with on the *top height and right position*
    n_8  = neighbour(coord, nx = +1, ny =  0, nz = -1) # neighbor of the *previous slide* with on the *same height and right position*
    n_9  = neighbour(coord, nx = +1, ny = +1, nz = -1) # neighbor of the *previous slide* with on the *bottom height and right position*
    n_10 = neighbour(coord, nx = -1, ny = -1, nz =  0) # neighbor of the *current slide* with on the *top height and left position*
    n_11 = neighbour(coord, nx = -1, ny =  0, nz =  0) # neighbor of the *current slide* with on the *same height and left position*
    n_12 = neighbour(coord, nx = -1, ny = +1, nz =  0) # neighbor of the *current slide* with on the *bottom height and left position*
    n_13 = neighbour(coord, nx =  0, ny = -1, nz =  0) # neighbor of the *current slide* with on the *top height and middle position*
    n_14 = neighbour(coord, nx =  0, ny = +1, nz =  0) # neighbor of the *current slide* with on the *bottom height and middle position*
    n_15 = neighbour(coord, nx = +1, ny = -1, nz =  0) # neighbor of the *current slide* with on the *top height and right position*
    n_16 = neighbour(coord, nx = +1, ny =  0, nz =  0) # neighbor of the *current slide* with on the *same height and right position*
    n_17 = neighbour(coord, nx = +1, ny = +1, nz =  0) # neighbor of the *current slide* with on the *bottom height and right position*
    n_18 = neighbour(coord, nx = -1, ny = -1, nz = +1) # neighbor of the *next slide* with on the *top height and left position*
    n_19 = neighbour(coord, nx = -1, ny =  0, nz = +1) # neighbor of the *next slide* with on the *same height and left position*
    n_20 = neighbour(coord, nx = -1, ny = +1, nz = +1) # neighbor of the *next slide* with on the *bottom height and left position*
    n_21 = neighbour(coord, nx =  0, ny = -1, nz = +1) # neighbor of the *next slide* with on the *top height and middle position*
    n_22 = neighbour(coord, nx =  0, ny =  0, nz = +1) # neighbor of the *next slide* with on the *same height and middle position*
    n_23 = neighbour(coord, nx =  0, ny = +1, nz = +1) # neighbor of the *next slide* with on the *bottom height and middle position*
    n_24 = neighbour(coord, nx = +1, ny = -1, nz = +1) # neighbor of the *next slide* with on the *top height and right position*
    n_25 = neighbour(coord, nx = +1, ny =  0, nz = +1) # neighbor of the *next slide* with on the *same height and tight position*
    n_26 = neighbour(coord, nx = +1, ny = +1, nz = +1) # neighbor of the *next slide* with on the *bottom height and tight position*
    ...
    if connect == 4: # 2D, only surfaces connected
        ...
        connect_voxels_coordinates = np.vstack((n_2, n_4, n_5, n_6,  
                                               n_8, n_10, n_11, n_12, n_13, n_14,
                                               n_15, n_16, n_17, n_19, n_21, 
                                               n_22, n_23, n_25))
        ...
        connect_voxels_coordinates = np.vstack((n_1, n_2, n_3, n_4, n_5, n_6, n_7, 
                                               n_8, n_9, n_10, n_11, n_12, n_13, n_14,
                                               n_15, n_16, n_17, n_18, n_19, n_20, n_21, 
                                               n_22, n_23, n_24, n_25, n_26))
...
def main_tort(_image: np.ndarray, _direction_flow: int, _starting_depth: int = 0, 
              _connection_type: int =26, _plotting_type: bool = False, _fname: str ='TestFig', 
    ...
    # print(f'!!!!!!!!!!{image_orig.shape=}')
    
    # skeletonize image
    medial_axis_image, nrows, ncolumns, ndepth, cum_number_medial_each_slide, number_medial_each_slide, skeleton_volume_axis_index = skeletonize3D(_image, _direction_flow, _plot_skel=_plotting_type, _fname=_fname, _DIRname=_DIRname)
    
    # find junctions (prestep for finding starting points)
    junction_index = find_junctions(image_orig, ncolumns, nrows, ndepth)
    
    # find starting points
    SP_loc_rad_shuffle = find_starting_points(_image, junction_index, _starting_depth, nrows, ncolumns, _plot_SP=_plotting_type, _fname=_fname, _DIRname=_DIRname)
    
    # find lengths of paths
    all_paths_length = running_paths(_image, SP_loc_rad_shuffle, nrows, ncolumns, ndepth, cum_number_medial_each_slide, number_medial_each_slide, skeleton_volume_axis_index, _connect_search=_connection_type, _plot=False, _fname=_fname, _DIRname=_DIRname)
    
    # get statistical values
    tortuosity_mean, tortuosity_std, tortuosity_median = statistical_tort(all_paths_length, nrows, _plot=_plotting_type, _SP_loc_rad=SP_loc_rad_shuffle,  _fname=_fname, _DIRname=_DIRname)
